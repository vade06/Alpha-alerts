...
# Market-wide SEC Form 4 scanner.
# We poll the SEC's current-filings Atom feed instead of polling
# every company individually.
SEC_CURRENT_FILINGS_URL = "https://www.sec.gov/cgi-bin/browse-edgar"

GLOBAL_FORM4_PAGE_SIZE = int(
    os.getenv("GLOBAL_FORM4_PAGE_SIZE", "100")
)

GLOBAL_FORM4_MAX_PAGES = int(
    os.getenv("GLOBAL_FORM4_MAX_PAGES", "5")
)

GLOBAL_FORM4_BASELINE_PAGES = int(
    os.getenv("GLOBAL_FORM4_BASELINE_PAGES", "3")
)

GLOBAL_INSIDER_ALERT_MODE = os.getenv(
    "GLOBAL_INSIDER_ALERT_MODE",
    "meaningful"
).strip().lower()

GLOBAL_INSIDER_MIN_BUY_SCORE = int(
    os.getenv("GLOBAL_INSIDER_MIN_BUY_SCORE", "40")
)

GLOBAL_INSIDER_MIN_BUY_VALUE = float(
    os.getenv("GLOBAL_INSIDER_MIN_BUY_VALUE", "100000")
)

GLOBAL_INSIDER_MIN_SELL_VALUE = float(
    os.getenv("GLOBAL_INSIDER_MIN_SELL_VALUE", "5000000")
)
...
FINNHUB_SOCIAL_SENTIMENT_URL = "https://finnhub.io/api/v1/stock/social-sentiment"

STOCK_SCORE_CACHE_SECONDS = int(
    os.getenv("STOCK_SCORE_CACHE_SECONDS", "300")
)
...
stock_score_cache = {}
stock_score_cache_lock = threading.Lock()

# Finnhub social sentiment is not available on every API plan.
# If Finnhub returns 403 once, disable this component for the
# remainder of the process so Railway logs are not spammed.
finnhub_social_sentiment_available = True
finnhub_social_sentiment_lock = threading.Lock()
...
# =========================================================
# STOCK OPPORTUNITY / QUALITY ENGINE
# =========================================================
...
def get_recent_stock_samples(symbol, hours=48, limit=500):
    with get_database() as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                """
            ...
            return [
                ...
                for price, sampled_at
                in cursor.fetchall()
            ]


def calculate_momentum_score(quote):
    ...
    day_percent = safe_float(quote.get("dp"))
    ...
    if day_percent is None and previous_close and previous_close > 0:
        day_percent = (
            (current / previous_close) - 1
        ) * 100

    if day_percent is not None:
        # +/- 10% day move can swing this component
        # from 10 to 90 before high/low-position adjustment.
        score += max(
            -40.0,
            min(
                40.0,
    ...
    if (
        ...
        # Trading near the high is positive momentum,
        # near the low is negative.
        score += (
            range_position - 0.5
        ) * 20.0
...
def calculate_technical_score(
    ...
    samples = get_recent_stock_samples(
        symbol,
        hours=48,
        limit=500,
    )
    ...
    # Simple slope check across recent samples.
    recent = prices[-min(
        12,
        len(prices),
    ):]
...
def get_finnhub_basic_metrics(symbol):
    if not FINNHUB_API_KEY:
        return {}
...
def calculate_fundamental_score(
    ...
    metrics = get_finnhub_basic_metrics(
        symbol
    )
...
def get_finnhub_social_sentiment(
    ...
    global finnhub_social_sentiment_available

    if not FINNHUB_API_KEY:
        return None

    with finnhub_social_sentiment_lock:
        if not finnhub_social_sentiment_available:
            return None
    ...
    try:
        ...
        # 403 means the endpoint is not permitted for this key/plan.
        # Disable it once rather than logging the same error for
        # every stock analysed.
        if response.status_code == 403:
            with finnhub_social_sentiment_lock:
                finnhub_social_sentiment_available = False

            print(
                "Finnhub social sentiment unavailable for this API key/plan. "
                "Social scoring disabled; overall stock scores will use the "
                "remaining available components."
            )
        ...
        print(
            f"Finnhub social sentiment "
            f"{symbol} HTTP error: {exc}"
        )
        return None
    ...
    for item in rows:
        ...
        pos_mentions = safe_float(
            item.get("positiveMention")
        )

        neg_mentions = safe_float(
            item.get("negativeMention")
        )

        row_score = safe_float(
            item.get("score")
        )

        if pos_mentions is not None:
            positive += max(
                0.0,
                pos_mentions,
            )

        if neg_mentions is not None:
            negative += max(
                0.0,
                neg_mentions,
            )
...
def opportunity_label(score):
    ...
    return (
        "LOW QUALITY / BEARISH SETUP"
    )


def get_stock_opportunity_score(
    ticker,
    force_refresh=False,
):
    ticker = (
    ...
    if not force_refresh:
        with stock_score_cache_lock:
            ...
            if (
                cached
                and now - cached["time"]
                <= STOCK_SCORE_CACHE_SECONDS
            ):
                return dict(
                    cached["value"]
                )

    insider = get_stock_insider_features(
        ticker
    )
    ...
    # Insider database uses -100..+100.
    # Convert to neutral-centred 0..100.
    insider_quality = clamp_score(
        50.0
        + insider_raw / 2.0
    )
    ...
    result = {
        ...
        "insider_features": insider,
    }

    with stock_score_cache_lock:
        stock_score_cache[ticker] = {
            "time": now,
            "value": dict(result),
        }
...
def get_sec_company_maps():
    ...
    # Prefer a shorter/simple ticker first where one issuer
    # has multiple listed share classes.
    ...
    return ticker_map, cik_map


def get_sec_ticker_map():
    ticker_map, _ = (
        get_sec_company_maps()
    )

    return ticker_map


def get_primary_ticker_for_cik(
    cik_map,
    cik,
):
    try:
...
# =========================================================
# MARKET-WIDE SEC FORM 4 FEED
# =========================================================

GLOBAL_FORM4_STATE_KEY = (
    "global_form4_feed_baseline_v1"
)
...
def global_form4_baseline_done():
    return (
        get_bot_state(
            GLOBAL_FORM4_STATE_KEY,
            "0",
...
def get_current_form4_feed(
    start=0,
    count=None,
):
    if count is None:
        count = (
            GLOBAL_FORM4_PAGE_SIZE
        )
    ...
    return parse_current_form4_atom(
        response.content
    )
...
def resolve_form4_xml_url(
    ...
    """
    SEC filing directories expose index.json. Form 4 filing
    directories normally contain a single raw XML ownership
    document such as form4.xml, ownership.xml or primary_doc.xml.
    """

    if not filing_index_url:
        raise RuntimeError(
            "Missing SEC filing index URL."
        )
    ...
    for item in items:
        ...
        # Exclude ordinary XBRL support files if one happens
    ...
    if not candidates:
        raise RuntimeError(
            "No raw Form 4 XML document "
            "found in SEC filing directory."
        )
...
def baseline_global_form4_feed(
    cik_map,
):
    """
    First market-wide run:
    remember current Form 4 feed entries only.
    It does NOT download or parse old Form 4 documents.
    """

    print(
        "Creating market-wide Form 4 baseline..."
    )

    remembered = 0

    for page in range(
        GLOBAL_FORM4_BASELINE_PAGES
    ):
        start = (
            page
            * GLOBAL_FORM4_PAGE_SIZE
        ...
        filings = (
            get_current_form4_feed(
                start=start,
                count=GLOBAL_FORM4_PAGE_SIZE,
            )
        )
    ...
    print(
        f"Market-wide Form 4 baseline ready: "
        f"{remembered} current filings remembered. "
        f"No historical XML was parsed."
    )


def should_alert_marketwide_form4(
    role,
    transactions,
    final_score,
    cluster,
    ...
    meaningful_purchase = (
        ...
        and (
            final_score
            >= GLOBAL_INSIDER_MIN_BUY_SCORE
            or buy_value
            >= GLOBAL_INSIDER_MIN_BUY_VALUE
            or senior_insider
            or cluster.get(
                "insiders",
                0,
...
def check_global_form4_filings(
    cik_map,
):
    """
    Market-wide real-time-ish Form 4 scanner.

    One SEC Atom feed is paged until we reach filings that
    are already known. Only new listed-company filings are
    resolved to raw XML and analysed.
    """

    total_new = 0
    processed = 0
    skipped_unlisted = 0
    reached_seen_history = False

    for page in range(
        GLOBAL_FORM4_MAX_PAGES
    ):
        start = (
            page
            * GLOBAL_FORM4_PAGE_SIZE
        ...
        filings = get_current_form4_feed(
            start=start,
            count=GLOBAL_FORM4_PAGE_SIZE,
        )
        ...
        for filing in filings:
            ...
            # Remember every filing immediately so malformed
            # or non-listed entries cannot loop forever.
            save_sec_filing(
                accession,
                ticker or "",
                company or "",
            ...
            if not ticker:
                skipped_unlisted += 1
                print(
                    f"GLOBAL FORM 4 SKIP: "
                    f"CIK {filing['issuer_cik']} "
                    f"has no SEC ticker mapping."
                )
                continue

            if not sec_filing_is_fresh(
                filing["filing_date"]
            ):
                print(
                    f"OLD GLOBAL FORM 4 "
                    f"REMEMBERED - NO ALERT: "
                    f"{ticker} | "
        ...
        # Once the feed page has no unseen entries, older pages
        # are already known as well in normal operation.
        if page_new == 0:
            break

        if (
            reached_seen_history
            and page_new
            < GLOBAL_FORM4_PAGE_SIZE
        ):
            break
    ...
    print(
        f"Global Form 4 scan: "
        f"{total_new} new filings | "
        f"{processed} listed filings analysed | "
        f"{skipped_unlisted} without ticker mapping."
    )
...
def process_form4_filing(ticker, company, filing, send_alert=True, market_wide=False):
    ...
    if market_wide:
        if not should_alert_marketwide_form4(
            role,
            transactions,
            final_score,
            cluster,
            ...
            print(
                f"GLOBAL FORM 4 STORED - LOW SIGNAL: "
                f"{ticker} | {insider} | "
                f"score {final_score:+d}"
            )
            return {
                "alerted": False,
                "score": final_score,
    ...
    try:
        opportunity = get_stock_opportunity_score(
            ticker,
            force_refresh=True,
        )
        ...
        print(
            f"Opportunity score {ticker} error: {exc}"
        )

    if opportunity:
        fields.append({
            "name":"Overall Stock Opportunity",
            "value":(
                f"**{opportunity['overall_score']:.0f}/100** - "
                f"**{opportunity['label']}**\n"
                f"Insider quality: **{format_component_score(opportunity['insider_quality'])}**\n"
                f"Technical setup: **{format_component_score(opportunity['technical_score'])}**\n"
                f"Momentum: **{format_component_score(opportunity['momentum_score'])}**\n"
                f"Fundamentals: **{format_component_score(opportunity['fundamental_score'])}**\n"
                f"Social sentiment: **{format_component_score(opportunity['social_score'])}**\n"
                f"Available evidence: **{opportunity['available_component_count']}/5 components**"
            ),
            "inline":False,
        })

        if opportunity.get("strongest_component"):
            fields.append({
                "name":"Bot Interpretation",
                "value":(
                    f"Strongest area: **{opportunity['strongest_component'].title()}**. "
                    f"Weakest available area: **{opportunity['weakest_component'].title()}**. "
                    f"The overall score measures setup quality, not a guaranteed probability of a price rise."
                ),
                "inline":False,
            })
    ...
    if opportunity:
        alert_description = (
            f"**{company} ({ticker})**\n\n"
            f"{interpretation}\n"
            f"Overall setup: **{opportunity['overall_score']:.0f}/100 - "
            f"{opportunity['label']}**"
        )
    else:
        alert_description = (
...
def monitor_main():
    ...
    if SEC_USER_AGENT:
        try:
            ...
            if not global_form4_baseline_done():
                baseline_global_form4_feed(
                    sec_cik_map
                )

            print(
                "Historical Form 4 backfill: OFF"
            )

            print(
                "Market-wide SEC Form 4 scanner: ON"
            )

            print(
                f"SEC-listed ticker mappings loaded: "
                f"{len(sec_ticker_map)}"
            )

            print(
                f"SEC alerts limited to filings no more than "
                f"{SEC_ALERT_MAX_AGE_DAYS} days old."
            )

            print(
                f"Global insider alert mode: "
                f"{GLOBAL_INSIDER_ALERT_MODE}"
            )
    ...
    print(
        f"Price-alert stock watchlist: "
        f"{len(STOCK_SYMBOLS)}"
    )

    print(
        f"Market-wide insider monitoring: "
        f"{'ON' if sec_cik_map else 'OFF'}"
    )
    ...
    while True:
        ...
        # Price-move alerts remain on the configured watchlist.
        # We do not call Finnhub for every listed US stock each
        # cycle because that would be wasteful and rate-limit heavy.
        if (
            FINNHUB_API_KEY
            and now - last_stock_check
        ...
        # Market-wide SEC Form 4 monitoring.
        if (
            SEC_USER_AGENT
            and sec_cik_map
            and now - last_sec_check
            >= SEC_CHECK_INTERVAL
        ):
            try:
                print(
                    "Checking market-wide "
                    "SEC Form 4 feed..."
                )

                check_global_form4_filings(
                    sec_cik_map
                )

            except Exception as exc:
                print(
                    f"Global SEC insider "
                    f"monitor error: {exc}"
                )
...
async def status(interaction: discord.Interaction):
    ...
    embed.add_field(name="SEC Insider", value="Market-wide" if SEC_USER_AGENT and PRIVATE_INSIDER_WEBHOOK else "Disabled", inline=True)
    ...
    with finnhub_social_sentiment_lock:
        social_status = (
            "Enabled"
            if finnhub_social_sentiment_available
            else "Unavailable on API plan"
        )
...
@bot.tree.command(name="insider", description="Show insider and overall stock quality")
...
async def insider(interaction: discord.Interaction, ticker: str):
    ...
    try:
        ...
        embed = discord.Embed(
            title=(
                f"{ticker} Stock Opportunity"
            ),
            description=(
                f"**Overall: {overall:.0f}/100**\n"
                f"**{opportunity['label']}**\n\n"
                f"This is a setup-quality score, "
                f"not a probability guarantee."
            ),
            color=color,
        )

        embed.add_field(
            name="Insider Quality",
        ...
        embed.add_field(
            ...
            value=(
                f"{opportunity['available_component_count']}/5"
            ),
            inline=True,
        )
        ...
        if opportunity.get(
            ...
            embed.add_field(
                name="Quality Read",
                value=(
                    f"Strongest: **{opportunity['strongest_component'].title()}**\n"
                    f"Weakest available: **{opportunity['weakest_component'].title()}**"
                ),
                inline=False,
            )
        ...
        await interaction.followup.send(
            f"Couldn't load stock quality for `{ticker}`."
        )
...
async def watchlist(interaction: discord.Interaction):
    await interaction.response.send_message(
        ...
        + "\n**Insider monitoring:** Market-wide SEC Form 4 scanner "
        + "for SEC-mapped U.S. listed stocks."
    )
